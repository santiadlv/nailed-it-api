...
def test_get_service_price_by_id(test_app):
    ...
    response = test_app.post("/services/get/id", json=test_identifier)
    assert response.status_code == status.HTTP_200_OK
    assert response.json().get('message') == "Service retrieved successfully"
...
def test_get_price_of_non_existent_service(test_app):
    ...
    assert response.json().get('detail') == "Service with provided ID could not found"
