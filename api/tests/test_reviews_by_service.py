...
@pytest.mark.asyncio
async def test_get_all_reviews_by_service(test_app):
    test_review = {
    ...
    ser_id = test_review["service_id"]
    response = test_app.get(f"/reviews/service/{ser_id}")
    data = (response.json().get('data'))
    size = len(data) #GET THE LENGTH OF THE LIST 
    print(size)

    # GET THE COUNT OF THE NUMBER OF DOCUMENTS INSIDE THE COLLECTION 
    count = await test_app.mongodb[settings.MONGODB_COLLECTION_REVIEWS].count_documents({"service_id": test_review["service_id"]})
